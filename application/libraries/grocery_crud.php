			...
			foreach($this->read_fields as $field_object)
			{
				...
				if(!isset($types[$field_name]))//Doesn't exist in the database? Create it for the CRUD
				{
					...
					$field_info = (object)array(
							...
							$this->display_as[$field_name] :
							ucfirst(str_replace("_"," ",$field_name)),
							'required'	=> !empty($this->required_fields) && in_array($field_info->name,$this->required_fields) ? true : false,
							'extras'	=> $extras
					);
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	////Added by Amit Shah - for default values set
	/**
	 * override get_layout
	 *
	...
	public function getStateInfo()
	{
		...
		switch ($state_code) {
			...
			case 18: // read
				...
				{
					throw new Exception('On the state "read" the Primary key cannot be null', 6);
					die();
				}
			break;
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	////Custom addition for default field values Ends
