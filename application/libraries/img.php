...
class img {
	...
	public function set_basepath($path) {
		//$this->ci->config->config['img_base_path'] = $path;
		$this->ci->config->set_item('img_base_path',$path);
	}
	
	public function set_baseurl($url) {
		//$this->ci->config->config['img_base_url'] = $url;
		$this->ci->config->set_item('img_base_url',$url);
	}
	/**
	 * main scale function
	 *
